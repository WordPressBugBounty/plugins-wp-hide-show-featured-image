...
add_filter('plugin_row_meta', 'whsfi_add_custom_plugin_links', 10, 2);
 ...
 *  Plugin Setting Link 
*/
if (! function_exists('whsfi_settings_link')) {
	function whsfi_settings_link($actions, $plugin_file)
	{
		static $plugin;
		...
		if ($plugin == $plugin_file) {
			...
			$settings = array('settings' => '<a href="' . admin_url('admin.php?page=whsfi_hide_options') . '">' . __('Settings', 'wp-hide-show-featured-image') . '</a>');
			$actions = array_merge($settings, $actions);
		}
		return $actions;
	}
}
add_filter('plugin_action_links', 'whsfi_settings_link', 10, 5);
						...
						<div class="inside">
							<form method="post" action="options.php">
								...
								<table class="form-table">
									<tr valign="top">
										<th scope="row"><?php _e('Hide Image from all posts(not Custom Post Type)?', 'wp-hide-show-featured-image'); ?></th>
										<td>
									...
									<tr valign="top">
										<th scope="row"><?php _e('Hide Image from all Pages?', 'wp-hide-show-featured-image'); ?></th>
										<td>
									...
									<tr valign="top">
										<th scope="row"><?php _e('Hide Admin Toolbar from the user end'); ?></th>
										<td>
									...
									<tr valign="top">
										<th scope="row"><?php _e('Remove the "Howdy" text in the upper right corner of your admin dashboard.'); ?></th>
										<td>
									...
									<tr valign="top">
										<th scope="row"><?php _e('Remove the WordPress logo from the upper left corner of the admin bar.'); ?></th>
										<td>
...
if ( ! empty( $whsfi_hide_howdy_text['howdy_text'] ) ) {
    ...
    add_action( 'wp_before_admin_bar_render', 'whsfi_remove_howdy', 25 );
}
 ...
 * @return $links [array]
 */
function whsfi_add_custom_plugin_links($links, $file)
{
	if ($file === 'wp-hide-show-featured-image/wp-hide-show-featured-image.php') {
		$links[] = '<a href="https://wp-plugins.galaxyweblinks.com/wp-plugins/wp-hide-show-featured-image/doc/" target="_blank">Documentation</a>';
